...
export default class ProductForm {
  ...
  onUploadImg() {
    this.subElements.productForm.uploadImage?.addEventListener('click', () => {
      const wrapper = document.createElement('div');
      wrapper.innerHTML = '<input type="file" accept="image/*" hidden>';
      const inputFile = wrapper.firstElementChild;
      ...
      inputFile.addEventListener('change', async () => {
        ...
        const newItem = this.getImageItem(result.data.link, imageFile.name);
        const ListImage = this.subElements.imageListContainer.firstElementChild

        ListImage.insertAdjacentHTML('beforeend', newItem);
